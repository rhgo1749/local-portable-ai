...
> * **권장 사양**: 모델에 따라 RAM 4GB ~ 16GB 이상 필요 (상세 내용은 아래 참조)
...
### 🛠️ 초기 설정
**`Local-Portable-AI\models`** 폴더에 호환되는 모델 폴더를 넣어줍니다.
   * **v0.6v 버전 기준 지원 모델 예시**:
     * `gemma-4-E2B-it-qat-UD-Q4_K_XL` — 권장 RAM 4GB 이상 (빠름, 긴 대화 길이, 낮은 성능, 이미지 지원)
     * `gemma-4-E4B-it-qat-UD-Q4_K_XL` — 권장 RAM 8GB 이상 (중간, 긴 대화 길이, 중간 성능, 이미지 지원)
     * `gemma-4-12B-it-qat-UD-Q4_K_XL` — 권장 RAM 16GB 이상 (느림, 중간 대화 길이, 고성능, 이미지 지원)
     * `gemma-4-26B-A4B-it-UD-IQ3_S` — 권장 RAM 16GB 이상 (중간, 짧은 대화 길이, 고성능, 이미지 미지원)

### 🚀 서버 가동 및 접속
1. 배포받은 **`Local-Portable-AI`** 폴더를 엽니다.
2. 폴더 내에 있는 **`run_server.bat`** 파일을 더블 클릭하여 실행합니다.
...
## 3. PDF 문서 분석 파이프라인 (v0.6v 기준)

v0.6v부터 PDF 분석 시 **하이브리드 파이프라인**이 자동으로 동작합니다. PDF의 종류에 따라 최적 경로를 선택합니다.
...
> [!NOTE]
> **비전 모드란?** `run_server.bat`에서 모델을 선택하면 자동으로 설정됩니다. E2B·E4B·12B 모델은 이미지 인식이 활성화되고, 26B 모델은 비활성화됩니다. 비전 모드에서는 시각 요소 분석에 추가 시간이 소요되지만 이미지나 삽화가 포함된 문서를 훨씬 정확하게 이해합니다.

### 🛡️ 하이브리드 파이프라인 및 폴백 (Fallback) 시스템

Tesseract OCR과 비전 모델 분석은 하이브리드(병렬/순차 큐) 구조로 동작하며, 비전 API가 타임아웃(페이지당 최대 20분 대기)이나 연결 실패 등으로 응답하지 않더라도 OCR 추출 결과는 정상적으로 보존되어 최종 마크다운에 통합됩니다.

```
[1️⃣ Tesseract OCR (텍스트 추출)]   [2️⃣ 비전 모델 (시각 이미지 분석)]
                 \                       /
                  \                     / (실패/타임아웃 시 제외)
                   ▼                   ▼
           [최종 결합 문서 마크다운 생성]
...
## 5. 프로젝트 구조 및 모듈화 아키텍처

본 프로젝트는 단일 스크립트(`mcp-bridge.js`)에서 주요 로직을 역할별로 분리하여 `src/` 폴더 하위의 모듈식 아키텍처로 개편되었습니다.

* **`mcp-bridge.js`**: 서비스 엔트리 포인트 (비차단식 스타트업 및 라우터 마운트)
* **`src/` 하위 모듈**:
  * **`router.js`**: CORS 제한 및 Completions API 프록싱, 스크립트 인젝션 라우팅
  * **`sandbox.js`**: 심볼릭 링크 우회 방어가 탑재된 보안 샌드박싱 검증
  * **`pdf-parser.js`**: Kordoc 및 PDFJS 기반 디지털/스캔 PDF 하이브리드 분석
  * **`ocr-vision.js`**: Tesseract.js 및 로컬 비전 API 병렬 추론 및 자동 워커 클린업
  * **`cache.js`**: 비동기 논블로킹 스타트업 클린업을 포함한 MD5 캐싱 엔진
  * **`globals.js`**: 전역 디버그 로깅 및 상태 관리
  * **`kordoc-loader.js`**: Kordoc 라이브러리 동적 로더
  * **`patcher.js`**: MCP 규격 호환을 위한 추론 응답 패치 모듈
...
* **NFC/NFD 한글 파일명 자소분리 정규화**: macOS 등에서 생성된 자소분리(NFD) 한글 파일명이 Windows 환경 보안 샌드박스 내부에서 오류 없이 검색 및 읽기가 가능하도록 항상 NFC 형식으로 자동 변환.
* **비규격 JSON 도구 호출 자동 보정**: 모델이 `tool_name` 대신 `tool_call`이나 `tool` 키를 사용하거나, `parameters` 없이 평탄(Flat)한 구조로 JSON을 직접 출력하더라도 자동으로 감지하여 표준 MCP 객체 구조로 변환 및 실행.
* **스키마 규격 파라미터 자동 정규화**: 모델이 잘못 요청한 변수명(`filePath` ↔ `path`, `content` ↔ `data` 등)을 각 도구별 스키마에 부합하도록 자동 매핑하여 WebUI 측 스키마 검증기 차단 오류 방지.
* **문서 비교 (`compare_documents`) 결과 개편**: 변경 사항이 없는 본문이나 테이블(`[테이블]`)의 무의미한 출력 나열을 필터링해 제외하고, 실제 변경된 항목의 전후 비교(`~ [변경] "이전" -> "이후"`)를 직관적으로 제공하도록 개선.
* **Completion 스트리밍 버퍼 최적화**: Completions API 스트리밍 수신 중 JSON 도구 호출 블록이 중간에 끊기거나 보정 타이밍을 놓치는 버그 방지를 위해 미완성 토큰 임시 버퍼 크기를 상향 조정.

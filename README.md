...
## 3. PDF 문서 분석 파이프라인 (v0.5 기준)

v0.5부터 PDF 분석 시 **하이브리드 파이프라인**이 자동으로 동작합니다. PDF의 종류에 따라 최적 경로를 선택합니다.
...
* **모듈화 아키텍처 도입**: 단일 파일 구조에서 `src/` 하위 모듈 구조로 개편하여 유지보수성 향상
* **Canvas 종속성 경량화**: `@napi-rs/canvas`를 제거하고 표준 `canvas` 라이브러리로 통합
* **환경 의존성 완화**: 시스템 진단 로직에서 PowerShell 의존성을 제거하고 `wmic` 및 `systeminfo` 순차 폴백 도입
* **보안성 대폭 강화**:
  * `fs.realpathSync`를 통한 심볼릭 링크 우회(Escape) 취약점 방어
  * CORS 와일드카드(`*`) 제거 및 Localhost(`127.0.0.1:8080`) Origin 제한으로 CSRF 위협 원천 차단
* **리소스 효율화 및 누수 보완**:
  * Tesseract OCR 워커 종료 시 프로세스 누수 자동 방지 (`SIGINT`/`exit` 클린업 등록)
  * PDF 문서 렌더링 완료 후 `doc.destroy()` 호출을 통해 메모리 누수 방지
  * 서버 부팅 시 캐시 정리(`cleanCache`) 프로세스를 비동기식(`fs.promises`)으로 전향하여 스타트업 지연 해결

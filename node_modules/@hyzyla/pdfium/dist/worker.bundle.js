...
const DEFAULT_PAGE_RENDER_OPTIONS = {
    scale: 1,
    render: "bitmap",
    colorSpace: "BGRA",
    ...
    render() {
        return __awaiter(this, arguments, void 0, function* (options = {}) {
            ...
            try {
                ...
                // Gray bitmaps have no alpha channel, so transparency only applies to BGRA.
                const fillColor = transparent && colorSpace === "BGRA" ? 0x00000000 : 0xffffffff;
    ...
    static initBase(options) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (wasmUrl) {
                ...
                // Also set a no-op locateFile so the vendor's findWasmBinary short-circuits
                // before its `new URL(..., import.meta.url)` call, which throws inside
                // blob-based worker contexts (issue #40). The returned URL is never read
...
class WorkerDispatcher {
    constructor(factory) {
        ...
        this.pages = new Map();
        this.pagesByDoc = new Map();
        this.factory = factory;
    }
    makeHandle(prefix) {
        return `${prefix}_${this.nextHandleId++}`;
    }
    ensureLibrary() {
        return __awaiter(this, void 0, void 0, function* () {
            if (this.library)
    ...
    getDoc(handle) {
        ...
        if (!doc)
            throw new Error(`Unknown document handle: ${handle}`);
        return doc;
    }
    getPage(handle) {
    ...
    disposeDocument(handle) {
        const doc = this.documents.get(handle);
        if (!doc)
            return;
        const docPages = this.pagesByDoc.get(handle);
        if (docPages) {
            for (const pageHandle of docPages)
                this.pages.delete(pageHandle);
            this.pagesByDoc.delete(handle);
        }
        doc.destroy();
    ...
    dispatch(req) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            switch (req.type) {
                case "init": {
                    // Init is handled by the worker entry layer, not by the dispatcher,
                    // because each flavor (base64/url/cdn) has its own bootstrap path.
                    throw new Error("init message must be handled by the worker entry, not the dispatcher");
                }
                case "loadDocument": {
                    const lib = yield this.ensureLibrary();
                    const doc = yield lib.loadDocument(req.bytes, (_a = req.password) !== null && _a !== void 0 ? _a : "");
                    const handle = this.makeHandle("d");
                    this.documents.set(handle, doc);
                    this.pagesByDoc.set(handle, new Set());
                    return { value: { doc: handle }, transfer: [] };
                }
                case "doc.getPageCount": {
                    return { value: this.getDoc(req.doc).getPageCount(), transfer: [] };
                }
                case "doc.getPage": {
                    const page = this.getDoc(req.doc).getPage(req.pageIndex);
                    const handle = this.makeHandle("p");
                    this.pages.set(handle, page);
                    (_b = this.pagesByDoc.get(req.doc)) === null || _b === void 0 ? void 0 : _b.add(handle);
                    return { value: { page: handle, pageIndex: req.pageIndex }, transfer: [] };
                }
                case "doc.destroy": {
                    this.disposeDocument(req.doc);
                    return { value: null, transfer: [] };
                }
                case "page.getText": {
                    return { value: this.getPage(req.page).getText(), transfer: [] };
                }
                case "page.getOriginalSize": {
                    return { value: this.getPage(req.page).getOriginalSize(), transfer: [] };
                }
                case "page.render": {
                    // Custom render functions can't cross postMessage; force "bitmap".
                    const renderParams = Object.assign(Object.assign({}, req.options), { render: "bitmap" });
                    const result = yield this.getPage(req.page).render(renderParams);
                    const value = {
                        width: result.width,
                    ...
                    return { value, transfer: [result.data.buffer] };
                }
                case "destroy": {
                    for (const handle of [...this.documents.keys()])
                        this.disposeDocument(handle);
                    (_c = this.library) === null || _c === void 0 ? void 0 : _c.destroy();
                    this.library = null;
                    this.initPromise = null;
                    return { value: null, transfer: [] };
                }
...
const dispatcher = new WorkerDispatcher(() => __awaiter(void 0, void 0, void 0, function* () {
    const init = yield initReady;
    return PDFiumLibrary.initBase({
        vendor: PDFiumModule,
        // instantiateWasm bypasses the vendor's findWasmBinary URL resolution,
        // which would otherwise crash inside a blob worker. We do our own
        // streaming instantiation here, with a fetch+compile fallback for servers
        // that serve pdfium.wasm with the wrong Content-Type.
        instantiateWasm: ((imports, success) => {
            const cb = success;
            void instantiate(init, imports).then(({ instance, module }) => cb(instance, module));
            return {};
        }),
    });
}));
function instantiate(init, imports) {
    return __awaiter(this, void 0, void 0, function* () {
        if (init.wasmBinary) {
            const result = yield WebAssembly.instantiate(init.wasmBinary, imports);
            return result;
        }
        if (!init.wasmUrl) {
            throw new Error("Worker init: wasmUrl or wasmBinary is required");
        }
        try {
            const result = yield WebAssembly.instantiateStreaming(fetch(init.wasmUrl), imports);
            return { instance: result.instance, module: result.module };
        }
        catch (_a) {
            const response = yield fetch(init.wasmUrl);
            const bytes = yield response.arrayBuffer();
            const result = yield WebAssembly.instantiate(bytes, imports);
            return result;
        }
    });
}
self.addEventListener("message", (event) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (req.type === "init") {
        if (!resolveInit) {
            const response = { id: req.id, type: "error", message: "Worker already initialized" };
            self.postMessage(response);
            return;
        }
        resolveInit({ wasmUrl: req.wasmUrl, wasmBinary: req.wasmBinary });
        resolveInit = null;
        const response = { id: req.id, type: "ok", value: null };
        self.postMessage(response);
        return;
    }
    try {
        const { value, transfer } = yield dispatcher.dispatch(req);
        const response = { id: req.id, type: "ok", value };
        self.postMessage(response, transfer);
    }
    catch (err) {
        const message = err instanceof Error ? err.message : String(err);
        const stack = err instanceof Error ? err.stack : undefined;
        const response = { id: req.id, type: "error", message, stack };
        self.postMessage(response);
    }
}));

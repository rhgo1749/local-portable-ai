...
function sheetToBlocks2(sheetName, sheet, sheetIndex) {
  ...
  for (let r = firstRow; r <= lastRow; r++) {
    ...
    for (let c = 0; c <= maxCol; c++) {
      ...
      row.push({
        text: grid[r][c],
        colSpan: _nullishCoalesce(_optionalChain([merge, 'optionalAccess', _48 => _48.colSpan]), () => ( 1)),
        rowSpan: _nullishCoalesce(_optionalChain([merge, 'optionalAccess', _49 => _49.rowSpan]), () => ( 1))
      });
    }
    cellRows.push(row);
  }
...
async function parseXlsDocument(buffer, options) {
  ...
  let pageFilter = null;
  if (_optionalChain([options, 'optionalAccess', _50 => _50.pages])) {
    const { parsePageRange: parsePageRange2 } = await Promise.resolve().then(() => _interopRequireWildcard(require("./page-range-3C7UGGEK.cjs")));
  ...
  for (let i = 0; i < totalSheets; i++) {
    ...
    _optionalChain([options, 'optionalAccess', _51 => _51.onProgress, 'optionalCall', _52 => _52(i + 1, totalSheets)]);
...
function lname(el) {
  return el.localName || _optionalChain([el, 'access', _53 => _53.tagName, 'optionalAccess', _54 => _54.replace, 'call', _55 => _55(/^[^:]+:/, "")]) || "";
}
...
function getChildElements(parent, localName3) {
  ...
  for (let i = 0; i < children.length; i++) {
    ...
    if (node.nodeType === 1) {
      const el = node;
      if (el.localName === localName3 || _optionalChain([el, 'access', _56 => _56.tagName, 'optionalAccess', _57 => _57.endsWith, 'call', _58 => _58(`:${localName3}`)])) {
        result.push(el);
      }
    }
  }
...
function findElements(parent, localName3) {
  ...
  const walk = (node) => {
    ...
    for (let i = 0; i < children.length; i++) {
      ...
      if (child.nodeType === 1) {
        const el = child;
        if (el.localName === localName3 || _optionalChain([el, 'access', _59 => _59.tagName, 'optionalAccess', _60 => _60.endsWith, 'call', _61 => _61(`:${localName3}`)])) {
          result.push(el);
        }
        walk(el);
      }
...
function collectOmmlRoots(p) {
  ...
  const walk = (node) => {
    ...
    for (let i = 0; i < children.length; i++) {
      ...
      if (child.nodeType !== 1) continue;
      const el = child;
      const tag = el.localName || _optionalChain([el, 'access', _62 => _62.tagName, 'optionalAccess', _63 => _63.replace, 'call', _64 => _64(/^[^:]+:/, "")]) || "";
...
function parseParagraph(p, styles, numbering, footnotes, rels) {
  ...
  if (_optionalChain([style, 'optionalAccess', _65 => _65.outlineLevel]) !== void 0 && style.outlineLevel >= 0 && style.outlineLevel <= 5) {
    return {
      type: "heading",
      text,
  ...
  if (numId && numId !== "0") {
    ...
    const levelInfo = _optionalChain([numDef, 'optionalAccess', _66 => _66.get, 'call', _67 => _67(ilvl)]);
    const listType = _optionalChain([levelInfo, 'optionalAccess', _68 => _68.numFmt]) === "bullet" ? "unordered" : "ordered";
  ...
  for (const tr of trElements) {
    ...
    for (const tc of tcElements) {
      ...
      for (const p of pElements) {
        ...
        if (_optionalChain([block, 'optionalAccess', _69 => _69.text])) cellTexts.push(block.text);
      }
  ...
  for (let c = 0; c < maxCols; c++) {
    for (let r = 0; r < rows.length; r++) {
      ...
      for (let nr = r + 1; nr < rows.length; nr++) {
        if (_optionalChain([rows, 'access', _70 => _70[nr], 'access', _71 => _71[c], 'optionalAccess', _72 => _72.rowSpan]) === 0) span++;
        else break;
      }
      cell.rowSpan = span;
    }
  ...
  for (const drawing of drawingElements) {
    ...
    for (const blip of blips) {
      ...
      try {
        ...
        imgIdx++;
        const ext = _nullishCoalesce(_optionalChain([imgPath, 'access', _73 => _73.split, 'call', _74 => _74("."), 'access', _75 => _75.pop, 'call', _76 => _76(), 'optionalAccess', _77 => _77.toLowerCase, 'call', _78 => _78()]), () => ( "png"));
        const mimeMap = {
          png: "image/png",
          jpg: "image/jpeg",
          jpeg: "image/jpeg",
...
async function parseDocxDocument(buffer, options) {
  ...
  for (let i = 0; i < children.length; i++) {
    ...
    const el = node;
    const localName3 = _nullishCoalesce(el.localName, () => ( _optionalChain([el, 'access', _79 => _79.tagName, 'optionalAccess', _80 => _80.split, 'call', _81 => _81(":"), 'access', _82 => _82.pop, 'call', _83 => _83()])));
    if (localName3 === "p") {
...
function parseHwpmlDocument(buffer, options) {
  ...
  const blocks = [];
  const pageFilter = _optionalChain([options, 'optionalAccess', _84 => _84.pages]) ? _chunkMUOQXDZ4cjs.parsePageRange.call(void 0, options.pages, countSections(body)) : null;
  let sectionIdx = 0;
...
function parseParagraph2(el, blocks, paraShapeMap, sectionNum) {
  ...
  if (!text) return;
  if (_optionalChain([shapeInfo, 'optionalAccess', _85 => _85.headingLevel]) != null) {
  ...
  for (const block of cloned) {
    ...
    for (let r = 0; r < block.table.rows; r++) {
      for (let c = 0; c < block.table.cols; c++) {
        const cell = _optionalChain([block, 'access', _86 => _86.table, 'access', _87 => _87.cells, 'access', _88 => _88[r], 'optionalAccess', _89 => _89[c]]);
        if (!cell) continue;
...
function fillTable(table, values, filled, matchedLabels, patternFilledCells) {
  ...
  for (let r = 0; r < table.rows; r++) {
    for (let c = 0; c < table.cols - 1; c++) {
      ...
      const newValue = values.get(matchKey);
      if (_optionalChain([patternFilledCells, 'optionalAccess', _90 => _90.has, 'call', _91 => _91(`${r},${c + 1}`)])) {
...
function resolveTheme(theme) {
  return {
    h1: _nullishCoalesce(_optionalChain([theme, 'optionalAccess', _92 => _92.headingColors, 'optionalAccess', _93 => _93[1]]), () => ( DEFAULT_TEXT_COLOR)),
    h2: _nullishCoalesce(_optionalChain([theme, 'optionalAccess', _94 => _94.headingColors, 'optionalAccess', _95 => _95[2]]), () => ( DEFAULT_TEXT_COLOR)),
    h3: _nullishCoalesce(_optionalChain([theme, 'optionalAccess', _96 => _96.headingColors, 'optionalAccess', _97 => _97[3]]), () => ( DEFAULT_TEXT_COLOR)),
    h4: _nullishCoalesce(_nullishCoalesce(_optionalChain([theme, 'optionalAccess', _98 => _98.headingColors, 'optionalAccess', _99 => _99[4]]), () => ( _optionalChain([theme, 'optionalAccess', _100 => _100.headingColors, 'optionalAccess', _101 => _101[3]]))), () => ( DEFAULT_TEXT_COLOR)),
    body: _nullishCoalesce(_optionalChain([theme, 'optionalAccess', _102 => _102.bodyColor]), () => ( DEFAULT_TEXT_COLOR)),
    quote: _nullishCoalesce(_optionalChain([theme, 'optionalAccess', _103 => _103.quoteColor]), () => ( DEFAULT_TEXT_COLOR)),
    /** quoteColor가 명시되었는지 — blockquote charPr 분기에 사용 (baseline 호환) */
    hasQuoteOption: _optionalChain([theme, 'optionalAccess', _104 => _104.quoteColor]) !== void 0,
    tableHeader: _nullishCoalesce(_nullishCoalesce(_optionalChain([theme, 'optionalAccess', _105 => _105.tableHeaderColor]), () => ( _optionalChain([theme, 'optionalAccess', _106 => _106.bodyColor]))), () => ( DEFAULT_TEXT_COLOR)),
    tableHeaderBold: !!_optionalChain([theme, 'optionalAccess', _107 => _107.tableHeaderBold])
  };
}
async function markdownToHwpx(markdown, options) {
  const theme = resolveTheme(_optionalChain([options, 'optionalAccess', _108 => _108.theme]));
...
function renderHtml(markdown, options) {
  const preset = _nullishCoalesce(_optionalChain([options, 'optionalAccess', _109 => _109.preset]), () => ( "default"));
  const css = PRESETS[preset] + (_nullishCoalesce(_optionalChain([options, 'optionalAccess', _110 => _110.extraCss]), () => ( "")));
  const body = md.render(markdown);
  const watermark = _optionalChain([options, 'optionalAccess', _111 => _111.watermark]) ? `<div class="watermark">${escapeHtml(options.watermark)}</div>` : "";
  const watermarkCss = _optionalChain([options, 'optionalAccess', _112 => _112.watermark]) ? `
    .watermark {
      position: fixed;
      top: 50%; left: 50%;
...
async function htmlToPdf(html, options) {
  ...
  try {
    ...
    const margin = _optionalChain([options, 'optionalAccess', _113 => _113.margin]);
    const pdf = await page.pdf({
      format: _nullishCoalesce(_optionalChain([options, 'optionalAccess', _114 => _114.pageSize]), () => ( "A4")),
      landscape: _optionalChain([options, 'optionalAccess', _115 => _115.orientation]) === "landscape",
      ...
      displayHeaderFooter: !!(_optionalChain([options, 'optionalAccess', _116 => _116.header]) || _optionalChain([options, 'optionalAccess', _117 => _117.footer])),
      headerTemplate: _nullishCoalesce(_optionalChain([options, 'optionalAccess', _118 => _118.header]), () => ( "<div></div>")),
      footerTemplate: _nullishCoalesce(_optionalChain([options, 'optionalAccess', _119 => _119.footer]), () => ( '<div style="font-size:8pt;width:100%;text-align:center;color:#777;"><span class="pageNumber"></span>/<span class="totalPages"></span></div>'))
    });
...
async function parse(input, options) {
  let buffer;
  const opts = typeof input === "string" && !_optionalChain([options, 'optionalAccess', _120 => _120.filePath]) ? { ...options, filePath: input } : options;
...
async function parseHwp(buffer, options) {
  try {
    ...
    if (isDistributionSentinel(markdown) && isComFallbackAvailable() && _optionalChain([options, 'optionalAccess', _122 => _122.filePath])) {
      try {

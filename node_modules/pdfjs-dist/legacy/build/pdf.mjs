  ...
  async #get(key, rawData) {
    ...
    try {
      ...
      if (image.type === "image/svg+xml") {
        ...
        throw new Error("NodeJS createImageBitmap bypass");
      }
      data.refCounter = 1;
    } catch (e) {
      warn(e);
...
function drawImageAtIntegerCoords(ctx, srcImg, srcX, srcY, srcW, srcH, destX, destY, destW, destH) {
  if (srcImg && srcImg.constructor && srcImg.constructor.name === "CanvasElement") {
    try {
      if (srcImg._canvas) {
        srcImg = srcImg._canvas;
      } else if (srcImg.canvas) {
        srcImg = srcImg.canvas;
      } else {
        const realCanvas = global.createCanvas(srcImg.width, srcImg.height);
        const realCtx = realCanvas.getContext('2d');
        const srcCtx = srcImg.getContext('2d');
        const imgData = srcCtx.getImageData(0, 0, srcImg.width, srcImg.height);
        const realImageData = new global.ImageData(new Uint8ClampedArray(imgData.data), imgData.width, imgData.height);
        realCtx.putImageData(realImageData, 0, 0);
        srcImg = realCanvas;
      }
    } catch (e) {
      console.error("[Canvas Convert Error] Failed:", e.message);
    }
  }
